...
import { GameLocationsCard, GenerationLocationsGroup } from '../../../../components/game-locations-card/game-locations-card';
import { GENERATION_ORDER, VERSION_TO_GENERATION } from '../../../../../../shared/constants/version-generation-map';
...
export class PokemonGameLocations {
  ...
  public encountersUrl = computed(() => this.pokemonDetailsData.pokemonLocationAreaEncountersUrl() ?? '');
  ...
  public generationGroups = computed<GenerationLocationsGroup[]>(() => {
    ...
    groups.sort((a, b) => {
      const orderA = Object.values(GENERATION_ORDER).find((g) => g.title === a.generationTitle)?.order ?? 999;
      const orderB = Object.values(GENERATION_ORDER).find((g) => g.title === b.generationTitle)?.order ?? 999;
